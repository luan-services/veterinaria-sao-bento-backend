...
app.get('/', async (ctx) => {
    ...
    const appointments = await appointmentsService.listByFilter(role, user.id, validation.data);
    return ctx.json({ appointments });
});
...
app.patch('/:id', async (ctx) => {
    ...
    const role = user.role ?? "USER";
    const updated = await appointmentsService.update(id, role, validation.data);
