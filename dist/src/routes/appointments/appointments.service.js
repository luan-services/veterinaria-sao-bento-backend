...
export const appointmentsService = {
    ...
    */
    ...
    async cancel(appointmentId, userId, userRole) {
    ...
    /*
        admin only services
    */
    async listByFilter(userRole, userId, filters) {
        if (userRole !== "ADMIN") {
    ...
    async update(appointmentId, userRole, data) {
        if (userRole !== "ADMIN") {
    ...
    async delete(appointmentId, userRole) {
        if (userRole !== "ADMIN") {
            throw new HTTPException(403, { message: "You are not allowed to access this route" });
        }
        const existingAppointment = await prisma.appointment.findUnique({
